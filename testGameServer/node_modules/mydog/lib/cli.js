...
class clientProxy {
    ...
    connectCb() {
        // register
        let loginInfo = {
            T: 1 /* define.Cli_To_Master.register */,
            cliToken: this.token
        };
    ...
    heartbeat() {
        ...
        this.heartbeatTimeout = setTimeout(function () {
            let heartBeatMsg = { T: 2 /* define.Cli_To_Master.heartbeat */ };
    ...
    request(msg, timeout, cb) {
        let reqId = this.reqId++;
        let data = { "T": 3 /* define.Cli_To_Master.cliMsg */, "reqId": reqId, "msg": msg };
...
function cli_list(opts) {
    ...
    connectToMaster(opts.host, opts.port, opts.token, function (client) {
        ...
        function requestList() {
            client.request({ "func": "list" }, 10, function (err, msg) {
                if (err) {
                    return abort(err);
                }
...
function cli_send(opts, argv) {
    ...
    let serverIds = [];
    let serverTypes = [];
    let endMsg = { "serverIds": [], "serverTypes": [], "argv": argv };
    if (opts.serverId) {
...
(y/n)[no] ?    `;
    confirm(msg, (yes) => {
        ...
        connectToMaster(opts.host, opts.port, opts.token, function (client) {
            waitMsg("connect ok! waiting");
            client.request({ "func": "send", "args": endMsg }, 600, function (err, data) {
                clearWait();
                console.log();
                client.close(false);
                if (err) {
