...
class ClientManager {
    ...
    addClient(client) {
        if (client.session) {
            this.app.logger("error" /* loggerLevel.error */, `${meFilename} the I_client has already been added, close it`);
            client.close();
            return;
        }
        this.app.clientNum++;
    ...
    handleMsg(client, msgBuf) {
        try {
            if (!client.session) {
                this.app.logger("error" /* loggerLevel.error */, `${meFilename} cannot handle msg before added, close it`);
                client.close();
                return;
            }
    ...
    doRemote(msg, session, cmdArr) {
        ...
        if (svr.serverType !== cmdArr[0] || svr.frontend) {
            this.app.logger("error" /* loggerLevel.error */, `${meFilename} illegal doRemote`);
            return;
        }
