...
class UnregSocket_proxy {
    ...
    registerTimeout() {
        ...
        this.registerTimer = setTimeout(function () {
            app.logger("error" /* loggerLevel.error */, `${meFilename} unregistered socket, register timeout, close it, ${self.socket.remoteAddress}`);
            self.socket.close();
        }, 5000);
    }
    onData(_data) {
        ...
        try {
            ...
            app.logger("error" /* loggerLevel.error */, `${meFilename} unregistered socket, JSON parse error, close it, ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        // The first packet must be registered
        if (!data || data.T !== 1 /* define.Monitor_To_Master.register */) {
            app.logger("error" /* loggerLevel.error */, `${meFilename} unregistered socket, illegal data, close it, ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        // Is it a server?
        if (data.serverToken) {
            if (data.serverToken !== serverToken) {
                app.logger("error" /* loggerLevel.error */, `${meFilename} unregistered socket, illegal serverToken, close it, ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            if (!data.serverInfo || !data.serverInfo.id || !data.serverInfo.host || !data.serverInfo.port || !data.serverInfo.serverType) {
                app.logger("error" /* loggerLevel.error */, `${meFilename} unregistered socket, illegal serverInfo, close it, ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            this.registerOk();
        ...
        if (data.cliToken) {
            if (data.cliToken !== cliToken) {
                app.logger("error" /* loggerLevel.error */, `${meFilename} unregistered socket, illegal cliToken, close it, ${socket.remoteAddress}`);
                socket.close();
                return;
            }
            this.registerOk();
        ...
        app.logger("error" /* loggerLevel.error */, `${meFilename} unregistered socket, illegal socket, close it, ${socket.remoteAddress}`);
        socket.close();
    }
    onClose() {
        clearTimeout(this.registerTimer);
        app.logger("error" /* loggerLevel.error */, `${meFilename} unregistered socket closed, ${this.socket.remoteAddress}`);
    }
    registerOk() {
        clearTimeout(this.registerTimer);
...
class Master_ServerProxy {
    ...
    init(data) {
        ...
        if (!!servers[data.serverInfo.id]) {
            app.logger("error" /* loggerLevel.error */, `${meFilename} already has a monitor named: ${data.serverInfo.id}, close it, ${socket.remoteAddress}`);
            socket.close();
            return;
        }
        ...
        app.logger("debug" /* loggerLevel.debug */, `${meFilename} get a new monitor named: ${this.sid}, ${this.socket.remoteAddress}`);
    }
    heartbeatTimeout() {
        this.heartbeatTimeoutTimer = setTimeout(() => {
            app.logger("error" /* loggerLevel.error */, `${meFilename} heartbeat timeout, close the monitor named: ${this.sid}, ${this.socket.remoteAddress}`);
            this.socket.close();
    ...
    onData(_data) {
        ...
        try {
            ...
            app.logger("error" /* loggerLevel.error */, `${meFilename} JSON parse error，close the monitor named: ${this.sid}, ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        try {
            if (data.T === 2 /* define.Monitor_To_Master.heartbeat */) {
                ...
                app.logger("error" /* loggerLevel.error */, `${meFilename} the monitor illegal data type close it: ${this.sid} ${this.socket.remoteAddress}`);
                this.socket.close();
            }
        }
        catch (e) {
            app.logger("error" /* loggerLevel.error */, e);
            this.socket.close();
        }
    }
    onClose() {
        ...
        app.logger("error" /* loggerLevel.error */, `${meFilename} a monitor disconnected: ${this.sid}, ${this.socket.remoteAddress}`);
    }
}
...
class Master_ClientProxy {
    ...
    init() {
        ...
        socket.on('data', this.onData.bind(this));
        socket.on('close', this.onClose.bind(this));
        app.logger("info" /* loggerLevel.info */, `${meFilename}  get a new cli: ${socket.remoteAddress}`);
    }
    heartbeatTimeOut() {
        this.heartbeatTimeoutTimer = setTimeout(() => {
            app.logger("error" /* loggerLevel.error */, `${meFilename} heartbeat timeout, close the cli: ${this.socket.remoteAddress}`);
            this.socket.close();
        }, define.some_config.Time.Monitor_Heart_Beat_Time * 1000 * 2);
    }
    onData(_data) {
        ...
        try {
            ...
            app.logger("error" /* loggerLevel.error */, `${meFilename} JSON parse error，close the cli: ${this.socket.remoteAddress}`);
            this.socket.close();
            return;
        }
        try {
            if (data.T === 2 /* define.Cli_To_Master.heartbeat */) {
                ...
                app.logger("info" /* loggerLevel.info */, `${meFilename} get command from the cli: ${this.socket.remoteAddress} ==> ${_data.toString()}`);
                masterCli.deal_cli_msg(this, data);
            }
            else {
                app.logger("error" /* loggerLevel.error */, `${meFilename} the cli illegal data type close it: ${this.socket.remoteAddress}`);
                this.socket.close();
            }
        }
        catch (e) {
            app.logger("error" /* loggerLevel.error */, `${meFilename} cli handle msg err, close it: ${this.socket.remoteAddress}\n ${e.stack}`);
            this.socket.close();
        }
    }
    send(msg) {
    ...
    onClose() {
        clearTimeout(this.heartbeatTimeoutTimer);
        app.logger("info" /* loggerLevel.info */, `${meFilename}  a cli disconnected: ${this.socket.remoteAddress}`);
    }
}
