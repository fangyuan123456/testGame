...
export class Pathfind {
    ...
    findPath(sx: number, sy: number, ex: number, ey: number, maxSearch?: number): tile[] {
        ...
        maxSearch = maxSearch || this.maxSearch;
    ...
    private getNeighbors(tile: tile): tile[] {
        ...
        // 左下
        if (x - 1 >= 0 && y - 1 >= 0 && tiles[y - 1][x - 1].val !== 0) {
            neighbors.push(tiles[y - 1][x - 1]);
        }

        // 右下
        if (y - 1 >= 0 && x + 1 <= max_x && tiles[y - 1][x + 1].val !== 0) {
            neighbors.push(tiles[y - 1][x + 1]);
        }

        // 左上
        if (y + 1 <= max_y && x - 1 >= 0 && tiles[y + 1][x - 1].val !== 0) {
            neighbors.push(tiles[y + 1][x - 1]);
        }

        // 右上
        if (y + 1 <= max_y && x + 1 <= max_x && tiles[y + 1][x + 1].val !== 0) {
            neighbors.push(tiles[y + 1][x + 1]);
        }
